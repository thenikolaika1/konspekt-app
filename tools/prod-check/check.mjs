...
const files = ["", "index.html", "service-worker.js", "styles.css?v=24", "styles.css", "app.js?v=24", "app.js",
  "js/classic-renderer.js?v=24", "js/classic-renderer.js", "js/mock-content.js?v=24", "js/store.js?v=24", "assets/thumbs/leaf.svg"];
const markers = { "": ["v=24", "v=23"], "index.html": ["v=24", "v=23"], "service-worker.js": ["konspekt-v24", "konspekt-v23"],
  "styles.css?v=24": ["CLASSIC NOTE", "КАМЕРА, ЗАКЛАДКИ", "tone-imp", "обложки конспектов"], "styles.css": ["CLASSIC NOTE"],
  "app.js?v=24": ["noteThumb", "inNoteSearch"], "app.js": ["noteThumb", "inNoteSearch"],
  "js/classic-renderer.js?v=24": ["cn-card", "qa-label", "remember-key"], "js/classic-renderer.js": ["cn-card"],
  "js/mock-content.js?v=24": ["assets/thumbs"], "js/store.js?v=24": ["migrateThumbnails"] };
say("=== HTTP ===");
...
say("=== FRESH USER FLOW ===");
{
  ...
  await p.evaluate((h) => scrollTo(0, Math.round((h - innerHeight) / 2)), hh); await p.waitForTimeout(300);
  await p.screenshot({ path: `${OUT}/10b-note-history-middle.png` });
  await p.evaluate(() => scrollTo(0, 1e6)); await p.waitForTimeout(300);
  await p.screenshot({ path: `${OUT}/10c-note-history-bottom.png` });
  say("history note DOM", await p.evaluate(() => ({ h2bg: getComputedStyle(document.querySelector(".cn-h2")).backgroundColor, cards: [...document.querySelectorAll(".cn-card")].map((x) => x.className + " " + getComputedStyle(x).backgroundColor), imp: getComputedStyle(document.querySelector(".cn-box.remember-key")).backgroundColor, answers: document.querySelectorAll(".qa-label").length })));
  await p.evaluate(() => scrollTo(0, 0));
  ...
  await p.click("[data-back]"); await p.waitForTimeout(300);
  await p.click("[data-go=bookmarks]"); await p.waitForTimeout(400);
  await p.screenshot({ path: `${OUT}/13b-all-bookmarks.png` });
  await p.click("[data-back]"); await p.waitForTimeout(300);
